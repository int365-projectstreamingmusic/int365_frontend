    ...
    <div class="flex flex-col w-full">
      <div class="flex justify-center h-full">
        <div>
            ...
            <div v-if="onePlaylist != ''"  class="sm:mx-10 mx-5 flex flex-row justify-between relative">
              <div class=" flex flex-row ">
                <div
                  ...
                  <img  v-if="onePlaylist != ''"
                    :src="url + 'api/streaming/image/' + onePlaylist.playlist.thumbnail"
                    class="xgl:w-44.5 xgl:h-52.5 md:w-36 md:h-40 sm:w-28 sm:h-32 w-20 h-24"
                    style=" object-fit: cover;" />
                </div>
                <div class="flex items-end ">
                  <div class="xgl:w-600 md:w-100 sm:w-54 w-40 xgl:ml-7 sm:ml-4 ml-2 xgl:mr-7 xgl:space-y-3 space-y-1 flex flex-col ">
                    <div
                    ...
                    <div class="font-sansation-regular md:text-ss ms:text-xs sm:text-ss text-mxs  text-blackTopFive sm:h-20 h-11 text-clip overflow-y-scroll">
              ...
              <div v-if="userId != null && onePlaylist.playlist.userAccountModel != undefined  && userId == onePlaylist.playlist.userAccountModel.accountId" class="cursor-pointer select-none z-30 static">
                <span class="material-icons active:text-violetdark hover:text-violetdark transition duration-200 flex justify-end lg:text-3xl md:text-2xl sm:text-xl text-lg"
            ...
            <div v-if="onePlaylist != '' "  class=" font-sansation-light sm:mx-10 mx-5 sm:mt-7 mt-4">
              <div class="2xl:w-1120 w-full text-center h-full">
                <div class="border-b-2 border-violetdark sm:text-sm text-xxs ">
                  <div class="2xl:w-1120  xl:w-full md:w-600 w-full sm:h-9 h-6 flex flex-row justify-center items-center">
                ...
                <div class="sm:text-sm text-xxs">
                  <div @click="acceptData(item)" v-for="(item, index) in onePlaylist.tracksInfo.content" :key="index"
                    ...
                    <div class="2xl:w-669 xl:w-504 md:w-74 w-36 md:px-3 sm:px-2 px-1 text-left  truncate ">{{ item.trackName }}</div>
          ...
          <div v-if="showPopupDel" class="fixed z-40 sm:top-0 sm:bottom-0 sm:left-0 sm:right-0  -top-65 bottom-0 left-0 right-0 font-sansation-light">
                ...
                <div class="border-2 border-gray-700 inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                  <div v-if="!loadingDel && (confirm || errorDel)" class="m-8">
                    ...
                    <div  class="flex justify-center">
                      <div @click="changeShowpopup('myplaylist')" class="w-36 h-12 cursor-pointer transition duration-200 rounded-md bg-violetdark text-white hover:shadow-xl hover:border-violet-800 hover:text-shadow-xl hover:border-2 flex justify-center item-center font-sansation-regular">
                        close
                      </div>
                    </div> 
                  </div>
                  ...
                  <div v-if="!loadingDel && !confirm && !errorDel" class="m-8">
                    ...
                    <div class="flex justify-center mt-5 " :class="textWrong? 'mb-3':'mb-5'">
                      <input v-model="message" placeholder="write your playlist name" :class="textWrong?'border-2 border-red-500 text-red-500':''" class="w-80 h-10 text-center hover-focus-input transition duration-200 rounded-md border-2 text-sm md:text-base" />
                    </div>
                    <div v-show="textWrong" class="text-sm text-red-500 text-center mb-3">error: does not match with text </div>
                    <div class="flex flex-row space-x-3 justify-center items-center select-none">
                      <div @click="changeShowpopup()" class="w-36 h-9 cursor-pointer transition duration-200 rounded-md bg-neutral-500 hover:shadow-xl hover:border-blackcoal hover:border-2 hover:text-shadow-xl  text-white flex justify-center item-center font-sansation-light">
                        cancel
                      </div>
                      <div @click="delToPlaylist()" class="w-36 h-9 cursor-pointer transition duration-200 rounded-md bg-red-500 hover:shadow-xl hover:border-red-700 hover:text-shadow-xl hover:border-2  text-white flex justify-center item-center font-sansation-ligth">
                        delete
                      </div>                      
                    </div> 
                  </div>
          ...
          <div v-if="authenticated && onePlaylist != '' && onePlaylist.playlist.userAccountModel != undefined && userId == onePlaylist.playlist.userAccountModel.accountId" class="font-sansation-light xgl:text-2xl md:text-xl text-lg text-blackcoal my-5 lg:mx-10">Playground</div>
          
          <div v-if="authenticated && onePlaylist != '' && onePlaylist.playlist.userAccountModel != undefined && userId == onePlaylist.playlist.userAccountModel.accountId" class="lg:mx-10 2xl:w-1200 xgl:w-962 md:w-698 sm:w-466 w-80 my-6 bg-gray-50 rounded-2xl h-fit sm:px-10 px-5 py-5 flex flex-col space-y-2 font-sansation-light">
            <div v-if="notfoundPG"
          ...
          <paginate v-show="authenticated && onePlaylist != '' && onePlaylist.playlist.userAccountModel != undefined && userId == onePlaylist.playlist.userAccountModel.accountId
            && playground != '' &&!notfoundPG" :totalItems="totalPG" :sizePage="totalPagePG" :itemsPerPage="10" :maxPagesShow="4"
            @pageNum="resPNComment"></paginate>
          <div class="2xl:w-1200 xgl:w-962 lg:mx-10 md:w-698 sm:w-466 w-80 mt-3 mb-20 ">
            ...
            <div v-if="authenticated"  >  
              <div class="flex flex-row items-center sm:space-x-4 space-x-2 mx-3 mb-5">
                <div class="overflow-hidden rounded-full shadow-lg md:w-13 md:h-13 w-10 h-10">
                  <img v-if="data != null"
                    :src="url + 'api/streaming/image/' + data.profileIamge"  class="md:w-13 md:h-13 w-10 h-10" style=" object-fit: cover;" />
                </div>
                <input type="text"
                ...
                <div @click="addOneComment()" class="select-none hover:text-white active:text-white text-blackcoal flex justify-center item-center bg-neutral-100 rounded-full md:h-10 h-7 sm:w-24 w-17 font-sansation-light md:text-base text-xs cursor-pointer transition duration-200 hover:bg-violetdark active:bg-violet-800">
                  <p>comment</p>
                </div>  
              </div>
            </div>
            <div >
              ...
              <div v-if=" comment != '' && !notfoundCM" >
                <div v-for="(onecomment) in comment" :key="onecomment.id" class="bg-neutral-50 rounded-lg p-3 my-2 h-full flex flex-row">
                  <div class="overflow-hidden rounded-full shadow-lg md:w-13 md:h-13 w-10 h-10">
                    <img v-if="comment != ''"
                    :src="url + 'api/streaming/image/' + onecomment.user.profileIamge" class="md:w-13 md:h-13 w-10 h-10" style=" object-fit: cover;" />
                  </div>
                  <div class="2xl:w-1120 xgl:w-962 md:w-600 sm:w-96 w-64 sm:pl-5 pl-2 font-sansation-light">
                    <div class="flex flex-row justify-between">
                      <div  class="flex flex-col space-y-1 ">
                        <div class="font-sansation-bold md:text-lg text-sm text-blackcoal">{{onecomment.user.username}}</div>
                      ...
                      <div class="flex flex-row space-x-0.5 select-none">
                        ...
                        <span @click="addReportComment(onecomment.id)" v-if="authenticated && onecomment.user.accountId != userId" class="cursor-pointer material-icons md:text-xl text-lg text-blackcoal hover:text-orange-500 active:text-orange-800 transition duration-200">report</span>
                        <span @click="editShow(onecomment.id,onecomment.comment)" :class="editTime && editId == onecomment.id ? 'text-violetdark' : ''"  v-if="authenticated && onecomment.user.accountId == userId" class="cursor-pointer material-icons md:text-xl text-lg text-blackcoal hover:text-violetdark active:text-violet-800 transition duration-200">edit</span>
                        <span @click="delComment(onecomment.id)" v-if="authenticated && onecomment.user.accountId == userId" class="cursor-pointer material-icons md:text-xl text-lg text-blackcoal hover:text-red-500 active:text-red-800 transition duration-200">close</span>
                      </div>
                    </div>
                    <div v-if="!(editId == onecomment.id)" class="md:text-base leading-tight sm:text-sm text-xxs pt-1">
                    {{onecomment.comment}}
                    </div>
                    <div v-if="editTime && editId == onecomment.id" class="md:text-base leading-tight sm:text-sm text-xxs pt-1">
                      <textarea 
                    ...
                    <div v-if="editTime && editId == onecomment.id" class="flex flex-row md:space-x-3 space-x-2 select-none">
                      <div @click="submitEditComment(onecomment.id)" class="md:w-20 sm:w-15 w-10 text-center  bg-blackcoal hover:bg-violetdark active:bg-violet-800 font-sansation-light text-white rounded-md cursor-pointer transition duration-200">
                      edit</div>
              ...
              <paginate v-show="comment != '' && !notfoundCM" :totalItems="totalComment.totalElements" :sizePage="totalComment.totalPages" :itemsPerPage="10" :maxPagesShow="4"
            @pageNum="resPNComment"></paginate>
            </div> 
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async delToPlaylist(){
      console.log(this.onePlaylist.playlist.playlistName )
